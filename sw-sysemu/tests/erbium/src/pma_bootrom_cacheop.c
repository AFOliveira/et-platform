...
/*
* Test: CacheOp allowed on BootROM, PrefetchVA blocked
*
* FlushVA on BootROM should succeed (no tensor_error).
* PrefetchVA on BootROM should fail (tensor_error bit 7 set),
* because PrefetchVA errors if storing to the address would fault (PRM-8).
*/
...
/* FlushVA value: dest=1 (L2), addr=BOOTROM_BASE, count=0 (1 line) */
...
int main() {
    /* FlushVA on BootROM — should succeed (CacheOp allowed) */
    clear_tensor_error();
    asm volatile("csrw %0, %1" :: "i"(CSR_FLUSH_VA), "r"(FLUSH_VA_BOOTROM));
    if (read_tensor_error() & TENSOR_ERROR_PMA)
        TEST_FAIL;

    /* PrefetchVA on BootROM — should fail (store would fault) */
    clear_tensor_error();
