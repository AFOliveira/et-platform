...
//------------------------------------------------------------------------------
// Erbium memory region address helpers. Bases come from hwinc/top.h
// (regenerated from the top-level RDL); upper bounds either come from the
// next region's base or, where the gap is intentionally large, are kept
// numeric with a comment.
...
{ return (addr >= ERBIUM_TOP_UART_REGISTERS_BASE) && (addr < ERBIUM_TOP_UART_REGISTERS_BASE + 0x1000ull); }

// BootROM is 8KB inside the ROMRAM region.
static inline bool paddr_is_bootrom(uint64_t addr)
{ return (addr >= ERBIUM_TOP_BOOTROM_BASE) && (addr < ERBIUM_TOP_BOOTROM_BASE + 0x2000ull); }

// SRAM is 4KB inside the ROMRAM region.
static inline bool paddr_is_sram(uint64_t addr)
{ return (addr >= ERBIUM_TOP_SRAM_BASE) && (addr < ERBIUM_TOP_SRAM_BASE + 0x1000ull); }
...
// PLIC: 64MB span (RISC-V PLIC spec maximum).
static inline bool paddr_is_plic(uint64_t addr)
{ return (addr >= ERBIUM_TOP_PLIC_BASE) && (addr < ERBIUM_TOP_PLIC_BASE + 0x4000000ull); }
