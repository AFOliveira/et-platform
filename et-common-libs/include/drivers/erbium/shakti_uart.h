 ...
 *
 * Shakti UART — stateless inline register-access wrappers used by
 * drivers/erbium/uart_platform.h. OS-agnostic; all functions take
 * a uintptr_t base. MMIO goes through isa/common/mmio.h; bitfield
 * extraction uses the GET/SET/MODIFY macros from the generated
 * hwinc/uart.h.
 */

#ifndef _DRIVERS_ERBIUM_SHAKTI_UART_H_
#define _DRIVERS_ERBIUM_SHAKTI_UART_H_
...
/* V3 TX trigger: after writing TX_REG, must write BAUD, DELAY, IEN,
 * RX_THRESHOLD to start transmission. */
static inline void shakti_uart_tx_trigger(uintptr_t base, uint16_t baud, uint32_t ien)
{
...
static inline void shakti_uart_int_enable(uintptr_t base, uint32_t *shadow, uint32_t mask)
{
    *shadow |= mask;
    shakti_uart_int_set(base, *shadow);
}

static inline void shakti_uart_int_disable(uintptr_t base, uint32_t *shadow, uint32_t mask)
{
    *shadow &= ~mask;
    shakti_uart_int_set(base, *shadow);
}
...
#endif /* _DRIVERS_ERBIUM_SHAKTI_UART_H_ */
