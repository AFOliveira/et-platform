 ...
 * address, read-modify-write, and poll-until-bits helpers. Not MMIO-specific;
 * ESR and other memory-mapped register paths use the same primitives once
 * the address is computed.
 *
 * Stateless, header-only, OS-agnostic.
 */

#ifndef _ISA_COMMON_MMIO_H_
#define _ISA_COMMON_MMIO_H_
...
static inline void reg_modify32(uintptr_t addr, uint32_t mask, uint32_t val)
{
...
static inline int reg_wait_for_bits(uintptr_t addr, uint32_t mask,
...
static inline int reg_wait_bit_set(uintptr_t addr, uint32_t bit, uint32_t max_iter)
{
    return reg_wait_for_bits(addr, bit, bit, max_iter);
}

static inline int reg_wait_bit_clear(uintptr_t addr, uint32_t bit, uint32_t max_iter)
{
    return reg_wait_for_bits(addr, bit, 0, max_iter);
}

#endif /* _ISA_COMMON_MMIO_H_ */
