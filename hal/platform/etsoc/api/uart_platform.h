 ...
 * view — RX/TX data at offset 0x0, line status at offset 0x14.
 *
 * This backend normalizes that model to the generic poll-mode helpers
 * in <api/uart.h>. Baud programming and Erbium-style pin-mux gating
 * are not exposed; those helpers are no-ops / unsupported.
 */

#ifndef ETSOC_API_UART_PLATFORM_H_
#define ETSOC_API_UART_PLATFORM_H_
...
/*
 * sys_emu's RX file-descriptor model reports DR=1 at EOF, so polling
 * LSR.DR directly can report a stale "data ready" after the last byte
 * was consumed. Cache one byte here to normalize poll-mode semantics
 * for tests.
 */
...
#endif /* ETSOC_API_UART_PLATFORM_H_ */
