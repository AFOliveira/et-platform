...
static inline uint32_t platform_plic_read_priority(uint32_t src)
{
	return reg_read32(_PLIC_BASE + _PLIC_PRIORITY_BASE + src * 4);
}

static inline void platform_plic_write_priority(uint32_t src, uint32_t val)
{
	reg_write32(_PLIC_BASE + _PLIC_PRIORITY_BASE + src * 4, val);
}

static inline uint32_t platform_plic_read_pending(uint32_t word)
{
	return reg_read32(_PLIC_BASE + _PLIC_PENDING_BASE + word * 4);
}

static inline uint32_t platform_plic_read_enable(uint32_t ctx, uint32_t word)
{
	return reg_read32(_PLIC_BASE + _PLIC_ENABLE_BASE + ctx * 0x80 + word * 4);
}

static inline void platform_plic_write_enable(uint32_t ctx, uint32_t word, uint32_t val)
{
	reg_write32(_PLIC_BASE + _PLIC_ENABLE_BASE + ctx * 0x80 + word * 4, val);
}

static inline uint32_t platform_plic_read_threshold(uint32_t ctx)
{
	return reg_read32(_PLIC_BASE + _PLIC_THRESHOLD_BASE + ctx * 0x1000);
}

static inline void platform_plic_write_threshold(uint32_t ctx, uint32_t val)
{
	reg_write32(_PLIC_BASE + _PLIC_THRESHOLD_BASE + ctx * 0x1000, val);
}

static inline uint32_t platform_plic_claim(uint32_t ctx)
{
	return reg_read32(_PLIC_BASE + _PLIC_CLAIM_BASE + ctx * 0x1000);
}

static inline void platform_plic_complete(uint32_t ctx, uint32_t src)
{
