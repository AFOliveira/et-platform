 ...
 *
 * Generic register access primitives — volatile load/store at a
 * physical address, read-modify-write, and poll-until-bits helpers.
 * Not MMIO-specific; ESR and other memory-mapped register paths use
 * the same primitives once the address is computed.
 *
...
static inline int reg_wait_bit_set(uintptr_t addr, uint32_t bit,
				     uint32_t max_iter)
{
	return reg_wait_for_bits(addr, bit, bit, max_iter);
}

static inline int reg_wait_bit_clear(uintptr_t addr, uint32_t bit,
				       uint32_t max_iter)
{
